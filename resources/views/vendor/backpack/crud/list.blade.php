...
@section('header') 
	...
	<section class="content-header">
	  <h1>
	    <span>{{ mb_ucfirst($crud->entity_name_plural) }}</span>
	  ...
	  <ol class="breadcrumb">
	    <li><a href="{{ url($crud->route) }}">{{ mb_ucfirst($crud->entity_name_plural) }}</a></li>
...
@section('after_scripts')
	...
	<script type="text/javascript">
              ...
              if (confirm("{{ trans('backpack::crud.delete_confirm') }}") === true) {
                  $.ajax({
                      ...
                      success: function(result) {
                          ...
                          var tr = delete_button.closest('tr');

                          if (tr.attr('role') !== 'row') {
                              tr.prev('[role="row"]').remove();
                          }
      ...
      @endif
			...
			function calculatePartnerComission() {

				var totalPartnerComissionContainer = jQuery('#total_partner_comission_container');

				if (!!totalPartnerComissionContainer) {
					getTotalPartnerComission(totalPartnerComissionContainer);
				}
			}

			function getTotalPartnerComission(container) {

				var totalPartnerComission = 0;

				jQuery( ".partner_comission_amount" ).each(function( index ) {
					var amount = parseInt(jQuery(this).html());

					totalPartnerComission += amount;
				});

				container.html(totalPartnerComission);
			}
	  });
	</script>
