...
<li filter-name="{{ $filter->name }}" filter-type="{{ $filter->type }}" class="dropdown @if(Request::get($filter->name)) active @endif">
		...
		@if (is_array($filter->values) && count($filter->values))
			@foreach($filter->values as $key => $value)
				@if ($key === 'dropdown-separator')
					<li role="separator" class="divider"></li>
				@else
					<li class="@if($filter->isActive() && $filter->currentValue == $key) active @endif">
