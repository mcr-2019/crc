  <div class="form-group col-md-12 image" data-preview="#{{ $field['name'] }}" data-aspectRatio="{{ isset($field['aspect_ratio']) ? $field['aspect_ratio'] : 0 }}" data-crop="{{ isset($field['crop']) ? $field['crop'] : false }}" @include('crud::inc.field_wrapper_attributes')>
    ...
    <div class="row">
        <div class="col-sm-6" style="margin-bottom: 20px;">
            @php
                $existing_value = isset($field['value']) ? $field['value'] : (isset($field['default']) ? $field['default'] : '');
            @endphp
            <img id="mainImage" src="{{ old($field['name']) ? old($field['name']) : (isset($field['prefix']) && ! empty($existing_value) ? url($field['prefix'].$existing_value) : ( ! empty($existing_value) ? url($existing_value) : '')) }}">
        </div>
