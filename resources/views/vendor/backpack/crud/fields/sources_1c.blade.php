...
?>
<div ng-app="sources1cApp" ng-controller="tableController" @include('crud::inc.field_wrapper_attributes') >
    ...
    <div class="array-container form-group">
        ...
        <table class="table table-bordered table-striped m-b-0" ng-init="field = '#{{ $field['name'] }}'; items = {{ $items }}; max = {{$max}}; min = {{$min}}; maxErrorTitle = '{{trans('backpack::crud.table_cant_add', ['entity' => $item_name])}}'; maxErrorMessage = '{{trans('backpack::crud.table_max_reached', ['max' => $max])}}'">
            ...
            <thead>
            <tr>
                <th style="font-weight: 600!important;">URL</th>
                <th style="font-weight: 600!important;">Логин</th>
                <th style="font-weight: 600!important;">Пароль</th>
            ...
            <tr ng-repeat="item in items" class="array-row">
                <td>
                    <input class="form-control" type="text" ng-model="item.config.base_url">
                </td>
                <td>
                    <input class="form-control" type="text" ng-model="item.config.login">
                </td>
                <td>
                    <input class="form-control" type="text" ng-model="item.config.password">
                </td>
                <td>
                    <button class="btn btn-sm btn-default" type="button" ng-click="removeItem(item);">
...
@if ($crud->checkIfFieldIsFirstOfItsType($field, $fields))
    ...
    @push('crud_fields_scripts')
        ...
        <script>
            window.sources1cApp = window.angularApp || angular.module('sources1cApp', ['ui.sortable'], function($interpolateProvider){
                $interpolateProvider.startSymbol('<%');
                $interpolateProvider.endSymbol('%>');
            });

            window.sources1cApp.controller('tableController', function($scope){
