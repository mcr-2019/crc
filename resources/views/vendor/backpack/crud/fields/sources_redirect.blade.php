...
?>
<div ng-app="sourcesRedirectApp" ng-controller="tableController" @include('crud::inc.field_wrapper_attributes') >
    ...
    <div class="array-container form-group">
        ...
        <table class="table table-bordered table-striped m-b-0" ng-init="field = '#{{ $field['name'] }}'; items = {{ $items }}; max = {{$max}}; min = {{$min}}; maxErrorTitle = '{{trans('backpack::crud.table_cant_add', ['entity' => $item_name])}}'; maxErrorMessage = '{{trans('backpack::crud.table_max_reached', ['max' => $max])}}'">
            ...
            <thead>
                <tr>
                    <th style="font-weight: 600!important;">URL переадресации</th>
            ...
            <tbody ui-sortable="sortableOptions" ng-model="items" class="table-striped">
                <tr ng-repeat="item in items" class="array-row">
                    <td>
                        <input class="form-control" type="text" ng-model="item.config.redirect_to">
                    </td>
                    <td>
...
@if ($crud->checkIfFieldIsFirstOfItsType($field, $fields))
    ...
    @push('crud_fields_scripts')
        ...
        <script>
            window.sourcesRedirectApp = window.angularApp || angular.module('sourcesRedirectApp', ['ui.sortable'], function($interpolateProvider){
                $interpolateProvider.startSymbol('<%');
                $interpolateProvider.endSymbol('%>');
            });

            window.sourcesRedirectApp.controller('tableController', function($scope){
